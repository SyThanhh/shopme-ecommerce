...
public class CheckoutService {
	...
	private static final int DIM_DIVISION = 139; // hang so delivery hang quy dinh
	...
	private float calculateShippingCost(List<CartItem> cartItems, ShippingRate shippingRate) {
		...
		for(CartItem item : cartItems) {
			...
			float dimWeight = (product.getLength() * product.getWidth() * product.getHeight()) / DIM_DIVISION;
