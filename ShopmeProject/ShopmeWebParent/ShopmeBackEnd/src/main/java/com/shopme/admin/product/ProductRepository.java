...
public interface ProductRepository extends PagingAndSortingRepository<Product, Integer> {
	...
	public Page<Product> findAllInCategory(@Param("categoryId") Integer categoryId,
			@Param("categoryIdMatch") String categoryIdMatch, Pageable pageable);
	...
	@Query("SELECT p FROM Product p WHERE (p.category.id = ?1 OR p.category.allParentIDs LIKE %?2%) AND "
			...
			+ " OR p.fullDescription LIKE %?3% " + "OR p.brand.name LIKE %?3% " + "OR p.category.name LIKE %?3%) ")
