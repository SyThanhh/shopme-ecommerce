...
//		Setting saveSetting = settingRepository.save(siteName);
		...
		Setting copyright = new Setting("COPYRIGHT", "Shopme - Copyright © Shopme", SettingCategory.GENERAL);
		
		settingRepository.saveAll(List.of(siteLogo, copyright));
		
		Iterable<Setting> inIterable = settingRepository.findAll();
		assertThat(inIterable).size().isGreaterThan(0);
	}
